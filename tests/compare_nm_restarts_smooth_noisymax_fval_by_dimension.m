...
optimizers(1).func_name = 'fminsearch_nm_restarts';
...
for i=1:case_count
    ...
    % Add noisy version
    noisy_objective = make_noisy_objective(test_cases(i).objective(1), 'max');
...
for d=1:dimension_count
    ...
    % Tick corrections
    if (dim == 12)
        plot_options(1).y_ticks = [1E-9 1E-7 1E-5 1E-3 1E-1 1E+1 1E+3];
        plot_options(2).y_ticks = [1E-4 1E-2 1 1E+2 1E+4 1E+6];
        plot_options(3).y_ticks = [1E-7 1E-5 1E-3 1E-1 1E+1 1E+3];
        plot_options(6).y_ticks = [1E-4 1E-2 1 1E+2 1E+4 1E+6];
    end

    % Printing
    print_options = struct();
    print_options.print_path = sprintf('out/compare_nm_restarts_smooth_noisymax_fval_by_dimension_r%d.tex', dim);
